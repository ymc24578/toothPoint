...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    while(fscanf(file, "%s", buf) != EOF) {
        ...
        case 'n':               /* newmtl */
            ...
            break;
		case 'm':  /*texure file's name*/
			fgets(buf, sizeof(buf), file);
			sscanf(buf, "%s %s", buf, buf);
			dir = glmDirName(model->pathname);
			model->materials[nummaterials].filepath = strcat(dir,strdup(buf));
...
nLenOfWideCharStr=MultiByteToWideChar(CP_ACP,0,model->materials[nummaterials].filepath,-1,NULL,0);
...
TextureImage[0]=auxDIBImageLoad(pWideCharStr);
			/*--------------------*/
			TextureImage[0]=auxDIBImageLoad(CA2W(model->materials[nummaterials].filepath));
			//TextureImage[0]=auxDIBImageLoad(MultiByteToWideChar(model->materials[nummaterials].filepath));
			glGenTextures(1, &model->materials[nummaterials].texture[0]);					// Create Two Texture
			glBindTexture(GL_TEXTURE_2D, model->materials[nummaterials].texture[0]);
 ...
 */
GLfloat
glmUnitize(GLMmodel* model,GLfloat setScale)
{
    GLuint i;
...
glmDraw(GLMmodel* model, GLuint mode)
{
    ...
    while (group) {
		...
		if (mode & GLM_TEXTURE)
		{
			glBindTexture(GL_TEXTURE_2D, model->materials[group->material].texture[0]);
		}
